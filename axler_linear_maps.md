...
- V and W are just the subspaces that the input/output are
	- the output doesn't necessarily cover entire range (see: surjectivity)

You can always make a unique linear map between two subspaces
- Using their basis
...
- Tu = Tv implies u = v
	- <=> null T = {0}
- Think: invertability
- if output is the same, then input can't map to different things
...
- Expressed in terms of w_i b/c that's the output space
	- i.e. matrix expresses T(input_bases) in terms of the linear comb of output_bases
	- i.e. columns of matrix are the transformed standard bases of V

## Matrix multiplication
If M(T) is to be representative of T then
- How should matrix multiplication act?
	- M(ST) should be = M(S)M(T)
	- i.e. mulitplication representative of composition
- How do we get this behavior?
	- Let T: U -> V
	- Let S: V -> W
	- Let M(S) = A and M(T) = C
...
V/U where V is a subspace of U:
- all subspaces of V parallel to U
